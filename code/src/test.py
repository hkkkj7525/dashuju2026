...
def main():
	...
	# 双模型集成：长历史模型 + 2024窗口模型，分数相加后选 Top5
	ensemble_model_dirs = [
		'./model/60_158+39',
		'./model/60_158+39_s7',
	]
	...
	scaler = joblib.load(os.path.join(ensemble_model_dirs[0], 'scaler.pkl'))
	...
	for model_dir in ensemble_model_dirs:
		...
		scores_total = scores if scores_total is None else scores_total + scores
		print(f'模型已加载: {model_dir}')

	order = np.argsort(scores_total)[::-1]
	...
	top5_stock_ids = [sequence_stock_ids[i] for i in order[:5]]
	output_df = pd.DataFrame({
		'stock_id': top5_stock_ids,
		'weight': np.full(5, 0.1999),
	})
	output_df.to_csv(output_path, index=False)

	print(f'预测日期: {latest_date.date()}')
	print(f'参与排序股票数: {len(ranked_stock_ids)}')
	print(f'Top5: {top5_stock_ids}')
