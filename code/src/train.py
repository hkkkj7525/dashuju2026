...
from featurework import create_ranking_dataset_vectorized
import joblib
import os
import json
...
def train_ranking_model(model, dataloader, criterion, optimizer, device, epoch, writer):
    ...
    for batch in tqdm(dataloader, desc=f"Training Epoch {epoch+1}"):
        ...
        if batch_loss is not None:
            ...
            batch_loss.backward()
            if not config.get('drop_clip', False):
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=max(cosine_epochs, 1), eta_min=1e-6)
